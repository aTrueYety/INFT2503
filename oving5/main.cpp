...
public:
  ...
  public:
    ...
    std::string short_type() const override { return short_color_string() + "Ki"; }
  ...
  public:
    ...
    std::string short_type() const override { return short_color_string() + "Kn"; }
    bool valid_move(int from_x, int from_y, int to_x, int to_y) const override {
      int dx = abs(from_x - to_x);
      int dy = abs(from_y - to_y);
  ...
  void show() {
    ...
    // content
    for (int row = squares[0].size() - 1; row >= 0; --row) {
        oss << "|";
        for (size_t col = 0; col < squares.size(); ++col) {
            string cell = "";
            if (squares[col][row]) {
                cell = squares[col][row]->short_type();
            }
            oss << setw(3) << cell << "|";
        }
        oss << endl;
